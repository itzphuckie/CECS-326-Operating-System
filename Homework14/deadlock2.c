...
int main () {
  ...
  for (i = 0; i < 5 ; i++ ) {
	...
	// Fixing the dealock by adding special case
	// When its = 4, the last person has to grab the next person and continue the process because the previous one was already locked 
        if(i == 4){
		semop(semid, &Waits[(i+1) % 5], 1);
		printf("Philosopher %d grabbing right chopstick\n", i);
        	sleep(1);
        	semop(semid, &Waits[i], 1);
       		printf("Philosopher %d grabbing left chopstick\n", i);
	}else{
       		semop(semid, &Waits[i], 1);
		printf("Philosopher %d grabbing left chopstick\n", i);
		sleep(1);
	...
	sleep(1);
        ...
        sleep(1);
        printf("Philosopher %d surrendering chopsticks\n", i);

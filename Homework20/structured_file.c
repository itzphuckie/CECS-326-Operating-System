...
/* Writing empty records into the file.
  numberRecords = number of records to write 
*/
void clearFile(int numberRecords)
{ 
  char buffer[numberRecords*sizeof(struct CarEntry)];

  for(int i = 0; i < numberRecords*sizeof(struct CarEntry); i++)
  {
    buffer[i] = '\0';
  }

  lseek(CarFile, 0, SEEK_SET);
  write(CarFile, buffer, sizeof(struct CarEntry)*numberRecords);
}
